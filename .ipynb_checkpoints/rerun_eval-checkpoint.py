...
from src.data import stoi, itos
from src.model import NLM, ModelConfig
from train import evaluate_and_save
...
def load_model_from_ckpt(ckpt_path: str, device: torch.device) -> Tuple[NLM, int]:
    ...
    state = payload['model']
    # Handle torch.compile checkpoints where keys are prefixed with '_orig_mod.'
...
def main():
    ...
    # Load hparams for eval configs
    hp_path = os.path.join(run_dir, 'hparams.json')
    if not os.path.exists(hp_path):
        print('hparams.json not found in run dir', file=sys.stderr)
        sys.exit(1)
    ...
    for step, ckpt_path in find_checkpoints(run_dir):
        ...
        # Wipe specific step dir if exists
        ...
        # Evaluate across tasks (RPN prompts are emitted from ArithmeticDataset now)
        for op in ['+', '*']:
